...
import { firstName, nickname, lastName, middleNames, fullName, gender } from './../../../src/generators/identification/person';
import * as Number from './../../../src/generators/math/number';
import sinon from 'sinon';


const boolStub = sinon.stub(Number, "bool");
...
describe('Person Generator', () => {
    ...
    it('Should pick a male gender', () => {
        boolStub.returns(true);
        const gndr = gender();
        expect(gndr).to.be.string;
        expect(gndr).to.be.equal('Male');
    });

    it('Should pick a female gender', () => {
        boolStub.returns(false);
        const gndr = gender();
        expect(gndr).to.be.string;
        expect(gndr).to.be.equal('Female');
    });
